...
public struct PlayPauseButton: View {
    ...
    public init(isPaused: Binding<Bool>, action: @escaping () -> Void) {
        self._isPaused = isPaused
        self.action = action
    }
    public var body: some View {
        GeometryReader { proxy in
            PlayPauseShape(isPlaying: !isPaused)
                .foregroundColor(.white)
                .accessibility(label: isPaused ? Text("Play") : Text("Pause"))
                ...
                .background(Theme.primaryBrand, in: Circle())
        }
    }
...
struct PlayButton_Previews: PreviewProvider {
    static var isPaused: Binding<Bool> = .init(get: { return true }, set: { _ in })
